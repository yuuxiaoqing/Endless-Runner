class Endgame extends Phaser.Scene{
    ...
    create(){
        ...
        .setInteractive()
        .on('pointerdown', ()=>{this.menu.setStyle({fill: '#fff', fontSize: '48px',fontFamily: 'Yeon Sung'}); this.scene.start('menuScene'); })
        .on('pointerover', ()=>{this.menu.setStyle({fill:'#fff', fontSize: '48px',fontFamily: 'Yeon Sung'});})
        .on('pointerout', ()=>{this.menu.setStyle({fill:'#000', fontSize: '36px',fontFamily: 'Yeon Sung'})});

        //restart button
        this.restart = this.add.text(width/2-100, height/2+230, "restart", {fill: '#000', fontSize: '48px',fontFamily: 'Yeon Sung'}).setOrigin(0.5)
        .setInteractive()
        .on('pointerdown',()=>{this.restart.setStyle({fill: '#fff', fontSize: '48px',fontFamily: 'Yeon Sung'});this.scene.start('playScene');})
        .on('pointerover',()=>{this.restart.setStyle({fill: '#fff', fontSize: '48px',fontFamily: 'Yeon Sung'});})
        .on('pointerout',()=>{this.restart.setStyle({fill: '#000', fontSize: '36px',fontFamily: 'Yeon Sung'});});
    }
    // update(){
